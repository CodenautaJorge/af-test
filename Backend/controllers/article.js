...
var controller = {
	...
	getArticles: (req, res) =>{
		...
		var query = Article.find({archiveDate: null});
	...
	getArchived: (req, res) =>{
		...
		var query = Article.find({archiveDate: { $ne: null }});
	...
	archive: (req, res) =>{
		...
		//Recogemos el id del artículo por la url
		var articleId = req.params.id;
		var newDate = new Date().toISOString();
		console.log(newDate);
			
		Article.findOneAndUpdate({_id: articleId}, {archiveDate: newDate}, {new: true}, (err, articleUpdated) =>{
			...
			if(!articleUpdated){
				return res.status(404).send({
					status: "error",
					message: "Error, no existe el artículo!!"
				});
			}

			//Si no hay ningún error obtenemos el artículo actualizado
